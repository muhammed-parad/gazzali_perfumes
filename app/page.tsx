...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (canvas) {
      ...
      if (context) {
        ...
        const render = () => {
          const img = images.current[seq.current.frame];
          if (img && img.complete) {
             const hRatio = canvas.width / img.width;
             const vRatio = canvas.height / img.height;
             const ratio  = Math.max(hRatio, vRatio);
             
             const shiftXRatio = window.innerWidth >= 768 ? 0.75 : 0.5;
  ...
  return (
    <main ref={containerRef} className="relative w-full overflow-hidden selection:bg-gold-accent-1/30">
      ...
      <section className="hero-container relative z-10 w-full h-screen overflow-hidden bg-matte-black">
        ...
        <div className="hero-text-container-mobile flex md:hidden absolute inset-0 z-10 flex-col items-center justify-center pointer-events-none px-6">
           ...
           <div className="relative z-20 flex flex-col mb-16 items-center drop-shadow-[0_0_20px_rgba(212,175,55,0.25)]">
             ...
             <button className="hero-btn-mobile opacity-0 pointer-events-auto mt-8 bg-[#C9A646] text-black font-sans font-semibold py-3.5 px-10 rounded-full text-[15px] tracking-wide hover:bg-[#D4AF37] transition-colors duration-300 shadow-[0_4px_20px_0_rgba(201,166,70,0.3)]">
                Shop Now
             </button>
           </div>
        </div>
      ...
      <section className="relative z-10 min-h-screen flex flex-col justify-center px-6 md:px-16 lg:px-24 py-24 bg-matte-black">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between w-full max-w-7xl mx-auto gap-16">
          <div className="w-full md:w-1/2 flex justify-center">
            <img 
              ...
              className="scroll-reveal object-contain max-h-[50vh] md:max-h-[70vh] drop-shadow-[0_0_40px_rgba(212,175,55,0.15)]"
            />
          </div>
      ...
      <section className="offer-section relative z-10 min-h-screen flex items-center justify-center">
        <div className="offer-content flex flex-col items-center text-center">
           
           <div className="relative mb-6 cursor-pointer group">
             {/* Rotating 30% OFF Badge */}
             <div className="absolute -top-10 -right-16 w-24 h-24 rounded-full bg-gold-gradient flex items-center justify-center animate-spin-slow shadow-[0_0_30px_rgba(212,175,55,0.4)] group-hover:shadow-[0_0_50px_rgba(212,175,55,0.7)] transition-shadow duration-500">
                <span className="text-matte-black font-sans font-bold text-lg leading-tight p-2 text-center">30%<br/>OFF</span>
             </div>
             
             <div className="flex flex-col items-center text-white">
                <span className="text-misty-grey/50 text-2xl line-through decoration-gold-accent-1/60 font-sans mb-2">₹999</span>
                <span className="text-7xl md:text-9xl font-serif font-semibold tracking-tighter shadow-white hover:text-gold-accent-2 transition-colors duration-500">₹699</span>
             </div>
           </div>

           <div className="flex items-center space-x-3 mt-4 bg-white/5 py-2 px-6 rounded-full border border-white/10">
              <div className="w-3 h-3 rounded-full bg-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.8)] animate-pulse"></div>
              <p className="text-misty-grey font-sans text-sm tracking-widest uppercase">👉 Limited stock available</p>
           </div>
        </div>
      </section>
      ...
      <section className="relative z-10 min-h-screen flex flex-col items-center justify-center px-4">
        ...
        <button className="magnetic-btn scroll-reveal group relative px-12 py-6 bg-white rounded-full overflow-hidden cursor-pointer shadow-2xl" type="button">
